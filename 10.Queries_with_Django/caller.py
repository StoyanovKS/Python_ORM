...
def order_books_by_year() -> str:
    ...
    return '\n'.join(f'{b.publication_year} year: {str(b)}' for b in ordered_books)


#print(order_books_by_year())

# Exam: 04. Delete Review by ID
#
...
# Exam: 05. Filter Authors by Nationalities
#
def filter_authors_by_nationalities(nationality: str) -> str:
    authors_by_nationality = (Author.objects
    ...
    result = [
        ...
        for a in authors_by_nationality
    ]
...
# Exam: 06. Filter Authors by Birth Year
#
def filter_authors_by_birth_year(start_year: int, end_year: int) -> str:
    ...
    return '\n'.join(f'{a.birth_date}: {str(a)}' for a in filtered_authors)
